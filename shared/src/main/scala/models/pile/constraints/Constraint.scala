...
object Constraint {
  ...
  def finiteTimes(numTimes: Int) = Constraint("finite-times", (src, tgt, cards, gameState) => {
  ...
  def specificRanks(ranks: Seq[Rank]) = Constraint(s"any-${ranks.map(_.value).mkString}", (src, tgt, cards, gameState) => {
