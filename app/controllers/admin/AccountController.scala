...
object AccountController extends BaseController {
  def accountList(q: String, sortBy: String) = AdminAction.async { implicit request =>
    AccountService.searchAccounts(q).map { accounts =>
      Ok(views.html.admin.accountList(q, sortBy, accounts))
    }
  }

  def accountDetail(id: UUID, sortBy: String) = AdminAction.async { implicit request =>
  ...
  def removeAccount(id: UUID) = AdminAction.async { implicit request =>
    Future {
