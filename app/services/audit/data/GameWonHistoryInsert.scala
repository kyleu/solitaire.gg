...
object GameWonHistoryInsert {
  ...
  def insert(id: UUID, data: JsValue): Future[String] = parse(id, data) match {
    ...
    case _ => Future.successful("Won: Skipped")
  }
  ...
  private[this] def queries(record: (UUID, UUID, UUID, UUID, Int, Int, Int, Int, Long, LocalDateTime)): Future[String] = {
    ...
    val q = UserStatisticsService.registerHistory(gameId, isWin = true, duration, moves, undos, redos, occurred, userId)
    q.flatMap { _ =>
      Database.execute(GameHistoryQueries.SetCountsAndCompleted(gameId, moves, undos, redos, score, occurred, "won")).map(_ => "Won: Inserted")
    }
  }
}
