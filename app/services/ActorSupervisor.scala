...
private class ActorSupervisor extends InstrumentedActor with Logging {
  ...
  override def receiveRequest = {
    ...
    case GetSystemStatus => sender() ! SystemStatus(games.keys.toList.sorted.map(_ -> Nil), connections.keys.toList.sorted.map(_ -> "?"))
  ...
  private def handleCreateGame(gameType: String, session: String, username: String, conn: ActorRef) {
    ...
    val actor = context.actorOf(Props(new GameService(id, gameType, seed, List((session, username, conn)))), "game:" + id)
    games(id) = actor
  }
