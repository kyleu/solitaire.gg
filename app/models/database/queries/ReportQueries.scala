...
object ReportQueries {
  private[this] def playerClause(userIds: Seq[UUID]) = if (userIds.isEmpty) { "" } else { s" where player in (${userIds.map(id => "?").mkString(", ")})" }
  ...
  case class GameCountForUsers(userIds: Seq[UUID]) extends Query[Map[UUID, Int]] {
    override def sql = s"select player, count(*) as c from games${playerClause(userIds)} group by player"
  ...
  case class WinCountForUsers(userIds: Seq[UUID]) extends Query[Map[UUID, Int]] {
    override def sql = s"select player, count(*) as c from games where status = 'win'${playerClause(userIds)} group by player"
    override def values = userIds
    override def reduce(rows: Iterator[Row]) = rows.map { row =>
      UUID.fromString(row.as[String]("player")) -> row.as[Long]("c").toInt
    }.toMap
  ...
  case class RequestCountForUsers(userIds: Seq[UUID]) extends Query[Map[UUID, Int]] {
    override def sql = s"select user_id, count(*) as c from requests${playerClause(userIds)} group by user_id"
    override def values = userIds
    override def reduce(rows: Iterator[Row]) = rows.map { row =>
